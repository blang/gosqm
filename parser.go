...
const (
	ContextVehicle         Context = "Vehicle/Unit"
...
func (p *Parser) parseVehicle(class *sqm.Class, veh *Vehicle) {
	for _, prop := range class.Props {
		...
		case "leader":
			veh.IsLeader = prop.Value == "1"
		case "player":
			veh.Player = prop.Value
		case "description":
			veh.Description = prop.Value
		case "presence":
			veh.Presence = prop.Value
		case "presenceCondition":
			veh.PresenceCond = prop.Value
		case "placement":
			veh.Placement = prop.Value
		case "age":
			veh.Age = prop.Value
		case "lock":
			veh.Lock = prop.Value
		...
		case "side":
			veh.Side = prop.Value
		case "forceHeadlessClient":
			veh.ForceHeadlessClient = prop.Value == "1"
		default:
			p.saveError(&UnkownPropertyError{
				ParentClass: class,
				Property:    prop,
